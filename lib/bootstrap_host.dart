...
import 'package:rootcause_qr_inspector/bootstrap.dart';
...
class _BootstrapHostState extends State<BootstrapHost> {
  ...
  void _retry({bool temporary = false}) {
    setState(() {
      _failure = null;
      _future = AppBootstrapper.initialize(temporary: temporary).catchError((
        Object error,
        StackTrace stack,
      ) {
        AppDiagnostics.instance.record(
          error,
          stack,
          area: temporary ? 'bootstrap_temporary' : 'bootstrap',
        );
  ...
  Widget build(BuildContext context) {
    ...
    if (failure != null) {
      return _StartupRecovery(failure: failure, retry: _retry);
    }
    return FutureBuilder<AppServices>(
      ...
      builder: (BuildContext context, AsyncSnapshot<AppServices> snapshot) {
        if (snapshot.hasData) {
          ...
          return RootCauseQrInspectorApp(
        ...
        if (snapshot.hasError && _failure != null) {
          return _StartupRecovery(failure: _failure!, retry: _retry);
        }
          ...
          home: Scaffold(
            body: Center(
              child: CircularProgressIndicator(
                semanticsLabel: 'Inicializando aplicación',
              ),
            ),
          ),
        );
                  ...
                  child: Padding(
                    ...
                    child: Column(
                      ...
                      children: <Widget>[
                        ...
                        AppText(failure.message, textAlign: TextAlign.center),
                        const SizedBox(height: 8),
                        SelectableText(
                          'Diagnóstico: ${failure.errorType} · ${failure.stackFingerprint}',
                        ),
                        ...
                        OutlinedButton.icon(
                          ...
                          label: const AppText(
                            'Abrir sin historial ni inventarios persistentes',
                          ),
                        ),
                        const SizedBox(height: 8),
                        TextButton.icon(
                        ...
                        TextButton.icon(
                          ...
                          label: const AppText('Copiar diagnóstico privado'),
                        ),
                        const SizedBox(height: 8),
                        const AppText(
