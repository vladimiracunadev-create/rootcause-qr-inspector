...
class SchemaMigrator {
  ...
  final StoreRef<String, Object?> _meta = StoreRef<String, Object?>(
    '_schema_meta',
  );

  Future<SchemaMigrationResult> migrate() async {
    final int initial =
        await _meta.record('schemaVersion').get(database) as int? ?? 1;
    int version = initial;
    ...
    await database.transaction((Transaction transaction) async {
      if (version < 2) {
        await _meta
            .record('v2MigratedAt')
            .put(transaction, DateTime.now().toUtc().toIso8601String());
        version = 2;
        applied.add(2);
      }
      if (version < 3) {
      ...
      if (version < 4) {
        await _meta
            .record('encryptionMetadataInDatabase')
