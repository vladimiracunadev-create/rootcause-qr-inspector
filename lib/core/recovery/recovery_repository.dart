  ...
  Future<List<RecoveryIssue>> load({bool unresolvedOnly = true}) async {
    final List<RecordSnapshot<String, Map<String, Object?>>> snapshots =
        await _store.find(
          _database.database,
          finder: Finder(
            sortOrders: <SortOrder>[SortOrder('detectedAt', false)],
          ),
        );
    return snapshots
        .map(
          (RecordSnapshot<String, Map<String, Object?>> item) =>
              RecoveryIssue.fromJson(item.value),
        )
        .where(
          (RecoveryIssue issue) =>
              !unresolvedOnly || issue.state == RecoveryIssueState.unresolved,
        )
        .toList(growable: false);
  }

  Future<void> mark(String id, RecoveryIssueState state) async {
    final Map<String, Object?>? current = await _store
        .record(id)
        .get(_database.database);
    if (current == null) return;
    final RecoveryIssue issue = RecoveryIssue.fromJson(
      current,
  ...
  Future<void> clearResolved() async {
    ...
    await _database.database.transaction((Transaction transaction) async {
      for (final RecordSnapshot<String, Map<String, Object?>> item
        ...
        if (issue.state != RecoveryIssueState.unresolved) {
          await _store.record(item.key).delete(transaction);
        }
      }
    });
  }
