...
class SettingsRepository {
  ...
  Future<AppSettings> load() async {
    final String themeName =
        await _preferences.getString('theme_mode') ?? 'system';
    final String languageName =
        await _preferences.getString('language') ?? 'system';
    ...
    if (flagsJson != null) {
      try {
        flags = FeatureFlags.fromJson(
          Map<String, dynamic>.from(jsonDecode(flagsJson) as Map),
        );
      } on Object {
    ...
    return AppSettings(
      ...
      language: AppLanguage.values.firstWhere(
        (AppLanguage item) => item.name == languageName,
      ...
      confirmBeforeOpen:
          await _preferences.getBool('confirm_before_open') ?? true,
      hideSensitiveValues:
          await _preferences.getBool('hide_sensitive_values') ?? true,
      ...
      clearClipboardSeconds:
          await _preferences.getInt('clear_clipboard_seconds') ?? 30,
      historyRetentionDays:
          await _preferences.getInt('history_retention_days') ?? 0,
      featureFlags: flags,
    );
  }
  ...
  Future<void> save(AppSettings value) async {
    await Future.wait(<Future<void>>[
      ...
      _preferences.setString(
        'feature_flags',
        jsonEncode(value.featureFlags.toJson()),
      ),
    ]);
  }
