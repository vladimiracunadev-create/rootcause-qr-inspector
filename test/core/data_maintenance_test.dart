...
void main() {
  ...
  test(
    'key rotation updates records and active metadata in one database transaction',
    () async {
      final AppDatabase database = await AppDatabase.openTemporary();
      addTearDown(database.close);
      final MemoryEncryptionKeyProvider provider =
          MemoryEncryptionKeyProvider();
      final PayloadCipher cipher = PayloadCipher(keyProvider: provider);
      final EncryptionMetadataRepository metadata =
          EncryptionMetadataRepository(database);
      final StoreRef<String, Map<String, Object?>> history =
          stringMapStoreFactory.store('scan_history');
      await metadata.saveActiveKeyId(PayloadCipher.currentKeyId);
      final String oldPayload = await cipher.encryptJson(<String, dynamic>{
        'id': 'one',
      });
      await history.record('one').put(database.database, <String, Object?>{
        'payload': oldPayload,
      });

      final EncryptionRotationResult result = await DataMaintenanceService(
        database,
        cipher,
        metadata,
      ).rotateEncryptionKey();
      final Map<String, Object?> rotated = (await history
          .record('one')
      ...
      expect(
        await cipher.decryptJson(rotated['payload']! as String),
  ...
  test(
    'rotation aborts before changing metadata when a payload is incomplete',
    () async {
      final AppDatabase database = await AppDatabase.openTemporary();
      addTearDown(database.close);
      ...
      final StoreRef<String, Map<String, Object?>> history =
          stringMapStoreFactory.store('scan_history');
      await metadata.saveActiveKeyId(PayloadCipher.currentKeyId);
      await history.record('broken').put(database.database, <String, Object?>{
        'id': 'broken',
      });
  ...
  test('failed rotation removes a newly generated orphan key', () async {
    ...
    final PayloadCipher cipher = PayloadCipher(keyProvider: provider);
    final EncryptionMetadataRepository metadata = EncryptionMetadataRepository(
      database,
    );
    final StoreRef<String, Map<String, Object?>> history = stringMapStoreFactory
        .store('scan_history');
    final StoreRef<String, Map<String, Object?>> inventory =
    ...
    await inventory.record('broken').put(database.database, <String, Object?>{
      'id': 'broken',
    });
