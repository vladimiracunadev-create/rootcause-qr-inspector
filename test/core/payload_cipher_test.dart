...
void main() {
  test(
    'versioned envelope round-trips without exposing clear values',
    () async {
  ...
  test('legacy envelope remains readable and can be upgraded', () async {
    ...
    final String current = await cipher.encryptJson(<String, dynamic>{
      'value': 'legacy-compatible',
    });
    final Map<String, dynamic> envelope = Map<String, dynamic>.from(
      jsonDecode(current) as Map,
    );
    ...
    expect(
      cipher.inspect(await cipher.upgradeEnvelope(legacy)).legacy,
      isFalse,
    );
  });
  test('a missing decryption key is never replaced silently', () async {
    ...
    final String encrypted = await cipher.encryptJson(<String, dynamic>{
      'value': 'protected',
    });
    ...
    await expectLater(
      cipher.decryptJson(encrypted),
      throwsA(isA<StateError>()),
    );
    await expectLater(
      cipher.encryptJson(<String, dynamic>{'new': 'value'}),
      throwsA(isA<StateError>()),
    );
    expect(provider.read(PayloadCipher.currentKeyId), completion(isNull));
  });
  ...
  test('tampered authenticated ciphertext is rejected', () async {
    ...
    await expectLater(
      cipher.decryptJson(jsonEncode(envelope)),
      throwsA(anything),
    );
  });
