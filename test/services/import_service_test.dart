...
void main() {
  test('history import previews valid, duplicate, and rejected records', () {
    ...
    final HistoryImportPreview preview = ImportService.parseHistoryBytes(
      bytes,
      existingIds: <String>{record.id},
    );
    expect(preview.valid, 1);
    expect(preview.duplicates, 1);
    expect(preview.rejected, 1);
    expect(
      preview.apply(<String>{record.id}, ImportStrategy.skipDuplicates),
      isEmpty,
    );
  });
  ...
  test(
    'duplicate records inside the same file are collapsed before import',
    () {
      final ScanRecord record = ScanRecord.manual(
        rawValue: 'same',
        format: 'QR',
