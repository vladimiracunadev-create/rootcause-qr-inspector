...
def patch_android() -> None:
    ...
    text = text.replace(
        'android:label="rootcause_qr_inspector"',
        'android:label="RootCause QR Inspector"',
    )
...
def patch_web() -> None:
    ...
    manifest.update(
        {
            "name": "RootCause QR Inspector",
            "short_name": "RootCause QR",
            "description": "Inspector local de QR con hallazgos explicables y evidencia exportable.",
            "display": "standalone",
    ...
    text = text.replace("<title>rootcause_qr_inspector</title>", "<title>RootCause QR Inspector</title>")
    text = text.replace(
        'content="rootcause_qr_inspector"',
        'content="RootCause QR Inspector"',
    )
...
def generate_platforms(force: bool, platforms: tuple[str, ...]) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="rootcause-qr-inspector-") as tmp:
        target = Path(tmp) / "rootcause_qr_inspector"
        run(
            [
                flutter,
                "create",
